...
#        tmp_ = sum(tmp_)/len(tmp_)
    ...
    def update_weights(self, model: nn.Cell, global_round,prototypes=None,proto_mask=None):
        ...
        if args.distill or args.fedprox_mu >0:
            global_model:nn.Cell = copy.deepcopy(model)
            # global_model.eval()
...
#            criteria_proaux = [OhemCELoss(0.7) for _ in range(4)]  # num_aux_heads=4
        ...
        # print(type(self.trainloader))
        # print(self.trainloader.get_dataset_size())
        # exit()
        initial_lr = args.lr
        scheduler_dict = {
...
####################
            ...
            if args.verbose:
                ...
                string = '| Global Round : {} | Local Epoch : {} | {} images\tLoss: {:.6f}'.format(
                    ...
                    global_round, iter+1, len(self.trainloader_idx), float(loss.asnumpy()))
                print(string)
