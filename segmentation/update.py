...
class LocalUpdate0(object):
    ...
    def train(self,epoch=0):
        ...
        for batch_idx, kkk in enumerate(range(2)):
            ...
            mem_before = get_gpu_memory_usage(int(0))
            (loss,loss_ce,loss_1_item,loss_pi_item,loss_pa_item,loss_con_item,loss_con_2_item),grads = grad_fn(images, labels)
            optimizer(grads)
            mem_after = get_gpu_memory_usage(int(0))
            print(f"Epoch {epoch} GPU {2} - batch_idx {batch_idx} - before: {mem_before} - after: {mem_after} - 显存变化: {mem_after - mem_before:.2f} MiB - Tag: {mem_after - mem_before>0}")
...
#            criteria_proaux = [OhemCELoss(0.7) for _ in range(4)]  # num_aux_heads=4
        ...
        scheduler_dict = {
            'step': nn.piecewise_constant_lr(
                ...
                learning_rates=[initial_lr, initial_lr * 0.1]
            ),
...
####################
        ...
        for iter in range(args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                (loss,loss_ce,loss_1_item,loss_pi_item,loss_pa_item,loss_con_item,loss_con_2_item),grads = grad_fn(images, labels)
                mem_after = get_gpu_memory_usage(int(0))
                print(f"Global Round {global_round} - Epoch {iter} - batch_idx {batch_idx} - before: {mem_before} - after: {mem_after} - 显存变化: {mem_after - mem_before:.2f} MiB - Tag: {mem_after - mem_before>0}")
                optimizer(grads)
...
#        tmp_ = sum(tmp_)/len(tmp_)
    ...
    def train(self, model, test_loader, train_loader):
        ...
        def forward_fn(images,labels):
            logits, feat_head, *logits_aux = model(images)
            labels_ = labels
            loss = criteria_pre(logits, labels_)
            # loss_aux = [crit(lgt, labels_) for crit, lgt in zip(criteria_aux, logits_aux)]
